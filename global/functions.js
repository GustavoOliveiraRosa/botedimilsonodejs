...
module.exports = (bot, utils, ytdl, config) => {
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            try {
                ...
                console.error(`Não posso entrar nesse canal de voz, filho da puta!: ${err}`);
            }
        } else {
            queue.musics.push(music);
            if (playlist) return;
            else return message.channel.send(`🎵 **${music.title}** adicionada a  fila de canturia.`);
        }
        return;
    }
    ...
    bot.play = (guild, music) => {
        ...
        if (!music) {
            ...
            return queue.textChannel.send(`🎵 A musica acabou, dê play em outra e aproveite!`);
        }
        ...
        queue.textChannel.send(`🎵 Agora eu vou tocar essa em homenagem ao chico bioca : **${music.title}**`);
    }
