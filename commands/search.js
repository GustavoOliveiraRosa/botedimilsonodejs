            ...
            try {
                ...
                let embed = new discord.RichEmbed()
                    ...
                    .setDescription(`**-=- Musicas encontradas-=-**\n${videos.map(video => 
                        `**${++index} -** ${video.title}`).join('\n')}\n\n🎵 Selecione o numero de  **1** a **10** em  **10 segundos** ou eu vazo daqui fdp`);
                ...
                try {
                    ...
                    if (err) undefined
                    return message.channel.send(utils.cmd_fail('⚠ Passou os 10 segundos já corno', `${config.prefix}search <music>`));
                }
                const videoIndex = parseInt(response.first().content);
                var video = await bot.youtube.getVideoByID(videos[videoIndex - 1].id);
            } catch (err) {
                console.error(err);
                return [message.delete(), utils.timed_msg(utils.cmd_fail(`${message.author}, Não achei uma musica com esse lixo de nome \`${searchString}\``, `${config.prefix}play <music/url>`), 5000)];
            }
        }
